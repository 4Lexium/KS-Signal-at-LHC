                    ...
                    bool use_metadata_totalW = true)
{
    ...
    for (auto &kv : histos[sname]) {
        ...
        if (hname == "dr_lep2_jj_unw") continue;   //hname == "dr_lep2_jj_unw" debug to    || hname == "m_l2jj" || hname == "m_lljj"
        TH1F *h = kv.second;
                     ...
                     TH1F *hZ)
{
    ...
    TH1F *h_sig = nullptr;
    {
        ...
        if (it_sig != group_histo_var.end()) {
            ...
            if (var == "m_lljj" || var == "m_l2jj" ) {
                leg->AddEntry((TObject*)0, "#times 5 10^{4}", " ");
            }
        }
    }
    ...
    if (var == "m_l2jj") {
        hs_bkg->SetMaximum(7.95e2);   //1.19e4 for 500fb: 1.99e5  for 3fb:1.39e3  // FNAL: 2.75e3     (SS: 2.99e3)
    } else if (var == "m_jj") {
        hs_bkg->SetMaximum(1.5e5);    //1.5e4   for 500fb:1.5e5   for 3fb:1.5e3
        hs_bkg->SetMinimum(9e-4);   //1.5e-3  for 500fb:1.5e-2   for 3fb:1.5e-4
    } else if (var == "m_lljj" ) {
        hs_bkg->SetMaximum(4.95e2);   //1.19e4 for 500fb:1.99e5    for 3fb:1.39e3   // FINAL: 2.75e3   (SS1.99e3)
        hs_bkg->SetMinimum(0.0);
    } else if (var == "dilep_q") {
        hs_bkg->SetMaximum(1.5e10);
        hs_bkg->SetMinimum(1.5e-2);   //1.5e-3  for 500fb:1.5e-2   for 3fb:1.5e-4
    } else if (var == "jet_pt") {
        hs_bkg->SetMaximum(5.5e7);    //1.5e7  for 500fb:1.5e8    for 3fb:1.5e6
        hs_bkg->SetMinimum(9e-4);
    } else if (var == "lep1_pt") {
        hs_bkg->SetMaximum(5.5e8);    //1.5e7  for 500fb:1.5e8    for 3fb:1.5e6
        hs_bkg->SetMinimum(9e-4);
    } else if (var == "lep2_pt") {
        hs_bkg->SetMaximum(5.5e7);    //1.5e7  for 500fb:1.5e8    for 3fb:1.5e6
        hs_bkg->SetMinimum(9e-4);
    ...
    if (var != "disp") {
        ...
        lat.SetTextFont(32);
        lat.DrawLatex(0.51, 0.73, "#sqrt{s} = 13.6 TeV, #scale[0.7]{#int} L = 500 fb^{-1}");   //29.0  3.55
    } else {
...
// main macro
void SIGvsBKG_data22() { 
    // silence ROOT noise: keep only serious errors
    gErrorIgnoreLevel = kFatal; //kError
    bool use_metadata_totalW = true;
    std::cout << "User Test 089:" << std::endl;
    std::string output_dir = "/home/alexanum/WORKSPACE/testing/MadGraph5/analysis/SIGvsBKG_RECO/Figures/SIGvsBKG_reco2_3fb_withdat_new";
    ...
    */
    ...
    for (auto &s : samples) {
        ...
        histos[sname]["m_jj"] = new TH1F(
            ...
            16, 50, 790  // adjust range/bins to your taste
        );
        if (s.type == "SIG") {
            int n_disp = disp_bins.size() - 1;
    ...
    {
        const std::string sigSampleName = "WR5000_N50_2L2J"; 
        auto itSample = histos.find(sigSampleName);
        if (itSample != histos.end()) {
            TH1F *h_lljj_incl = itSample->second["m_lljj"];      // inclusive lljj   (is filled regardless of good l2-l1 distinction)
            TH1F *h_lljj_sel  = itSample->second["m_lljj_sel"];  // selective lljj  (as strict as l2jj)

            if (h_lljj_incl && h_lljj_sel) {
                TH1F *h_lljj      = (TH1F*)h_lljj_incl->Clone("h_lljj_debug");
                TH1F *h_lljj_copy = (TH1F*)h_lljj_sel ->Clone("h_lljj_copy_debug");

                // Style inclusive vs selective
                h_lljj->SetLineColor(kRed+1);
                h_lljj->SetLineWidth(2);
                h_lljj->SetFillStyle(0);
                h_lljj->SetMarkerStyle(20);
                h_lljj->SetMarkerColor(kRed+1);
                h_lljj->SetMarkerSize(0.8);
                h_lljj_copy->SetLineColor(kBlue+1);
                h_lljj_copy->SetLineWidth(2);
                h_lljj_copy->SetFillStyle(0);
                h_lljj_copy->SetMarkerStyle(24);
                h_lljj_copy->SetMarkerColor(kBlue+1);
                h_lljj_copy->SetMarkerSize(0.8);
                h_lljj->SetTitle(";m(#it{lljj}) / GeV;event weight");
                double maxY = std::max(h_lljj->GetMaximum(), h_lljj_copy->GetMaximum());
                h_lljj->SetMinimum(0.0);
                h_lljj->SetMaximum(1.5 * maxY);
                TCanvas *c_lljj_debug = new TCanvas(
                    "c_lljj_inclusive_vs_selective",
                    "Debug: inclusive vs selective m(lljj), signal only",
                    800, 600
                );
                c_lljj_debug->cd();
                gPad->SetLogy(kFALSE);
                gPad->SetLogx(kFALSE);
                h_lljj->GetXaxis()->SetTitleSize(0.045);
                h_lljj->GetXaxis()->SetLabelSize(0.04);
                h_lljj->GetYaxis()->SetTitleSize(0.045);
                h_lljj->GetYaxis()->SetLabelSize(0.04);
                h_lljj->Draw("HIST");
                h_lljj_copy->Draw("HIST SAME");

                TLegend *leg_lljj = new TLegend(0.60, 0.70, 0.88, 0.88);
                leg_lljj->SetBorderSize(0);
                leg_lljj->SetFillStyle(0);
                leg_lljj->SetTextSize(0.04);
                leg_lljj->AddEntry(h_lljj,      "m(lljj) inclusive", "l");
                leg_lljj->AddEntry(h_lljj_copy, "m(lljj) selective", "l");
                leg_lljj->Draw();

                c_lljj_debug->SaveAs((output_dir + "/debug_lljj_inclusive_vs_selective_signal.png").c_str());
            } else {
                std::cerr << "DEBUG: m_lljj or m_lljj_sel missing for sample " << sigSampleName << std::endl;
            }
        } else {
            std::cerr << "DEBUG: signal sample " << "WR5000_N50_2L2J" << " not found in histos map.\n";
        }
    }
