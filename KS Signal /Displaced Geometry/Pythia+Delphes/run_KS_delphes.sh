...
PYTHIA_PATH=/storage/user/madgraph/MG5_aMC_v2_9_25/HEPTools/pythia8
DELPHES_PATH=/storage/user/madgraph/MG5_aMC_v2_9_25/Delphes
...
# Recommend Independantly running: [ /storage/user/madgraph/MG5_aMC_v2_9_25/bin/mg5_aMC /home/user/WORKSPACE/testing/MadGraph5/scripts/generate_KS_scan.mg5 ]
# rm -rf KS_WR_*
if [ ! -d "$BASE/$PROC" ]; then
  echo "Generating process directory $PROC"
  $MG5 /home/user/WORKSPACE/testing/MadGraph5/scripts/remastered/generate_KS_chained.mg5
else
...
# PDF:pSet = LHAPDF6:NNPDF23_lo_as_0130_qed
  ...
  # Patch run_card.dat
  # minimal pT(j)>10 (very mild) and no cut on the pseudorapidity
  sed -i '
    s/^ptj.*/ptj = 10.0/
    s/^etaj.*/etaj = -1/
...
# nano /storage/user/WORKBENCH/runs/KS_delphes_decay/bin/internal/run_delphes3
# grep delphes /storage/user/madgraph/MG5_aMC_v2_9_25/input/mg5_configuration.txt
...
# (reload) source /storage/user/madgraph/MG5_aMC_v2_9_25/Delphes/DelphesEnv.sh 
